...
GROUP_TOKENS = {
    group: {strip_accents(word) for word in words}
    for group, words in GROUP_BY_PRODUCT.items()
}

def assign_group(name: str) -> str | None:
    toks = set(tokenize(name))
    for group, keywords in GROUP_TOKENS.items():
        if toks & keywords:
            return group
    return None
...
# ─────────────────────────────────────────────────────────────────────────────
# 4) Mapeo de scrapers disponibles
# ─────────────────────────────────────────────────────────────────────────────
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Leer todos los CSV históricos + nuevos
    df_list = [pd.read_csv(fp, dtype=str) for fp in glob.glob(PATTERN_DAILY)]
