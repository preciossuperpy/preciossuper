# -*- coding: utf-8 -*-
"""
Scraper unificado de precios – versión GitHub Actions
...
def _open_sheet():
    ...
    # Leer la hoja existente y eliminar filas completamente vacías
...
def main(argv=None):
    ...
    for k in objetivos:
        sc = SCRAPERS[k]()
        filas = sc.scrape()
        sc.save_csv(filas)  # Guarda CSV en OUT_DIR
        registros.extend(filas)
        print(f"• {k:<10}: {len(filas):>5} filas")

    if not registros:
        print("Sin datos nuevos.")
        return 0

    # Leer todos los archivos CSV del día (pueden ser varios scrapers)
    archivos_csv = glob.glob(PATTERN_DAILY)
    if not archivos_csv:
        print("No se encontraron archivos CSV para combinar.")
        return 1

    df_all = pd.concat([pd.read_csv(f, dtype=str) for f in archivos_csv],
                       ignore_index=True, sort=False)
    
    # Normalizar grupo y precio
    df_all["Grupo"] = df_all["Grupo"].map(strip_accents).fillna("")
    df_all["Precio"] = pd.to_numeric(df_all["Precio"], errors="coerce")

    # Abrir la hoja de Google y combinar con datos anteriores
