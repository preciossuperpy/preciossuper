...
Autor: Diego B. Meza · Revisión: 2025-06-28
"""
...
def assign_category(name: str) -> Tuple[Optional[str], Optional[str]]:
    if not name:
        return None, None
    name_lower = name.lower()
    main_cat, sub_cat = None, None
    for cat in CATEGORY_RULES:
        if any(re.search(excl, name_lower) for excl in cat.get("exclude", [])):
            continue
        if any(re.search(pat, name_lower) for pat in cat.get("include", [])):
            main_cat = cat["name"]
            break
    if not main_cat:
        return None, None
    for cat in CATEGORY_RULES:
        if cat["name"] == main_cat:
            for sub in cat.get("subcategories", []):
                if any(re.search(pat, name_lower) for pat in sub.get("include", [])):
                    sub_cat = sub["name"]
                    break
            break
...
class SalemmaScraper(HtmlSiteScraper):
    ...
    def category_urls(self):
        ...
        for a in soup.find_all("a", href=True):
            href = a["href"].lower()
            if any(k in href for k in patterns): urls.add(urljoin(self.base_url, href))
        return list(urls)
    def parse_category(self, url):
        r = self.session.get(url, timeout=REQ_TIMEOUT); r.raise_for_status()
        soup = BeautifulSoup(r.content, "html.parser")
